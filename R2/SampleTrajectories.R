...
# file <- paste0("ParamSet_TBE.csv")
temp <- read.table(file,header=TRUE,sep=",")
setwd(mainDir)
      
if (samplename=="TestRun0") {
  params <- unlist(temp[1,])
} else if (samplename=="TestRun1") {
  temp <- subset(temp,obsprob<0.7)
  params <- unlist(temp[1,])
} 
rm(temp)
...
for (q in 1:(length(params.noic)+length(params.ic))) {
  if (names(coef(po1)[q]) %in% titleset) {
    ...
    } else if ( names(coef(po1)[q]) %in% c("beta_nh","lambda_l") ){
...
reports <- coef(po1,"obsprob") * x[,"cases"] *(x[,"cases"]>=0)
