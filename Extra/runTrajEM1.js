...
function traj_match (data, params, times, index, place) {
  ...
  /* Optimizer function using subplex method */
  optimizer.f = logLik;
...
function integrate (params, tLength, deltaT) {
  ...
  lsodaTem = lsoda.cwrap('run_me', "number", inputArray)
  buffer = lsoda._malloc(lengthBuffer * nByte)
  lsodaTem(lengthBuffer, buffer, ...N, ...params, deltaT)
  for (var i = 0; i < lengthBuffer; i++) {
    arr.push(lsoda.getValue(buffer + i * nByte, 'double'))
  }
  lsoda._free(buffer)
  return arr
}
...
lsoda.onRuntimeInitialized = main
